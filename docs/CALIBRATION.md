...
2. Turn **Calibration mode** on. This pauses headline VWC and dryback tracking. It starts a fresh twenty-reading capture window.
...
4. Wait for **Capture ready**. At the default 30-second interval, twenty fresh replies take about ten minutes. The RAW range across the window must be at most 10 counts, and the generic-response averages of the older and newer halves must be within 0.35 percentage points. At that interval the drift limit rejects a steady slide faster than about 4 percentage points per hour. A slower creep can still pass, so start the window only after drainage already looks finished. If it does not settle, investigate distribution, movement, contact or continuing drainage. Do not raise the spread limit to force a moving slab through.
...
An **Experimental pwEC estimate** switch enables only the Hilhorst-type model `bulk EC × εw(T) / (apparent permittivity − offset)`, where `εw = 80.3 − 0.37×(T−20)` from the slab temperature. At 25°C that is 78.45. It is off at every boot, requires checked in-range VWC at or above the minimum (default 55%), and withholds invalid results rather than clamping them to a plausible limit. The offset 4.1 is Hilhorst's mineral-soil starting value, not a measured constant for rockwool or coco. The 55% floor keeps the divisor out of the region where a small permittivity error explodes the result. It does not turn the model into a pore-EC measurement. Fit it against extracted pore solution across the moisture and EC range you run before interpreting it quantitatively.
