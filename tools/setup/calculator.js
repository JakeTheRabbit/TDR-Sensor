...
(function (root, factory) {
  ...
  function taperedPot(top, bottom, height) {
    positive(top, "Top diameter");
    positive(bottom, "Bottom diameter");
    positive(height, "Fill height");
    return (
      (Math.PI * height * (top * top + top * bottom + bottom * bottom)) / 12000
    );
  }
  function litres(value, unit) {
    ...
    if (!(unit in factors))
      throw new Error("Choose litres, US gallons or Imperial gallons.");
    return value * factors[unit];
  }
  function allocation(cubeL, baseL, plants, units) {
    nonnegative(cubeL, "Block volume");
    nonnegative(baseL, "Shared base volume");
    whole(plants, "Plants per unit");
    whole(units, "Number of units");
  ...
  function weighed(dry, wet, volumeL, density = 1) {
    nonnegative(dry, "Dry assembly mass");
    ...
    if (vwc > 100)
      throw new Error(
        "Calculated VWC exceeds 100%. Check volume, tare and units.",
      );
    return { waterMl, vwc };
  }
  function shot(volumeL, percent, emitters, flowLh) {
    positive(volumeL, "Allocated substrate volume");
    positive(percent, "Shot percent");
    if (percent > 100) throw new Error("Shot percent cannot exceed 100.");
    whole(emitters, "Emitters per plant");
    positive(flowLh, "Measured emitter flow");
    const ml = (volumeL * 1000 * percent) / 100;
    return { ml, seconds: ml / ((emitters * flowLh * 1000) / 3600) };
  }
  function dryback(peak, current) {
    positive(peak, "Peak VWC");
    nonnegative(current, "Current VWC");
    if (peak > 100 || current > 100) throw new Error("VWC cannot exceed 100%.");
    return {
      points: peak - current,
      relative: (100 * (peak - current)) / peak,
    };
  }
